...
@pytest.fixture()
def tmp_pack(tmp_path: Path) -> tuple[Path, dict]:
    """Kopiere Originaldatei in ein temporaeres Verzeichnis."""
...
def test_valid_original_files(tmp_pack: tuple[Path, dict]) -> None:
    rec_path, _ = tmp_pack
    errors = vvs.validate_files(rec_path)
    assert errors == []


def test_wrong_prompt_count(tmp_pack: tuple[Path, dict]) -> None:
    rec_path, rec = tmp_pack
...
def test_duplicate_id(tmp_pack: tuple[Path, dict]) -> None:
    rec_path, rec = tmp_pack
    rec = copy.deepcopy(rec)
...
def test_duplicate_text(tmp_pack: tuple[Path, dict]) -> None:
    rec_path, rec = tmp_pack
    rec = copy.deepcopy(rec)
...
def test_missing_order(tmp_pack: tuple[Path, dict]) -> None:
    rec_path, rec = tmp_pack
    rec = copy.deepcopy(rec)
...
def test_wrong_style_count(tmp_pack: tuple[Path, dict]) -> None:
    rec_path, rec = tmp_pack
    rec = copy.deepcopy(rec)
...
def test_empty_text(tmp_pack: tuple[Path, dict]) -> None:
    rec_path, rec = tmp_pack
    rec = copy.deepcopy(rec)
    rec["prompts"][0]["text"] = ""
    _write(rec_path, rec)
...
def test_wrong_locale(tmp_pack: tuple[Path, dict]) -> None:
    rec_path, rec = tmp_pack
    rec = copy.deepcopy(rec)
    rec["locale"] = "en-US"
    _write(rec_path, rec)
...
def test_invalid_duration_bounds(tmp_pack: tuple[Path, dict]) -> None:
    rec_path, rec = tmp_pack
    rec = copy.deepcopy(rec)
